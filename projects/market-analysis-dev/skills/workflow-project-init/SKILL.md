...
- 模板、项目级 Skill 和 `AGENTS.md` 必须声明“对应角色本人反馈”：当前角色在自己的固定任务独立交付，不能只由 `00 包工头`转述。
- 当前产物获批并完成一跳路由后，下一角色必须在自己的固定任务宣布入场、获批范围、预期产物和停止门；`00 包工头`只监督汇总，不代替角色报到或交付。
- 未获授权的角色不得提前报到；固定角色任务不可达、重复、繁忙或消息失败时，准确登记并报告阻塞，不得伪造角色反馈。

## 用户直达固定角色模板（强制）

- 未来项目的 `AGENTS.md`、项目级 Skill、共享角色 Skill 快照和 `workflow/state.yaml` 必须声明：超级无敌帅超超总在固定角色本人任务中明确项目、范围与动作时，视为该角色该工作单元的一次性入场或继续授权，无需 `00 包工头`转发，并具有 AIWorkFlow 内部最高业务调度优先级。
- 模板必须同时保留边界：直达指令不隐含待审产物通过、不解冻其他工作、不授权下游、不产生级联；角色先记录原状态、安全恢复点和返回点，完成后回到原队列或审核门。
- 指令跨角色、冲突冻结现场或属于下游产品/UI 变更时，接收角色直接路由正确固定角色并保留现场，不得越权代做；高风险动作仍须本次具体动作的单独明确授权。
- 初始化器和验证器必须生成并校验上述语义，避免新项目退化为必须经过 `00` 的单点路由。
...
- 无论 Profile、技术栈或项目阶段如何，每个项目都必须存在 `ui/`，并在 `project.yaml` 的 modules 中声明该路径。
- 新的 UI/UX 提示词、设计说明、原型、生成界面和视觉审核产物统一进入 `ui/`，同时在 `workflow/artifacts.yaml` 登记权威路径、版本、状态与哈希。
- 已进入审核门的旧产物在审核结束前不得为整理目录而中途移动；应先在 `ui/README.md` 建立索引，待审批闭环后再单独迁移并更新状态、事件和哈希。
- `ui/` 存在只表示目录契约有效，不代表提示词、设计或前端实现已获批准。
...
10. 每个项目必须创建并保留 `ui/README.md`；初始化器与校验器应保证 `ui/` 存在且被 manifest 声明。
