...
const svg = (w,h,name,body,extra="") => `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-labelledby="title desc" ${extra}>${style}<title id="title">${esc(name)}</title><desc id="desc">前端职业成长雷达发布完整性 v1.5 版本化设计原型</desc>${body}</svg>`;
...
function header(title, subtitle, surface) {
  ...
  b += ltx("truth-strip",1390,122,"CR-UI-002 v1.5",13,C.danger,700,"end");
  return b;
}
...
for(const page of ["04","05","06","08"]){const a=desktopPage(page);add(a.name,"desktop",a.surface,"redrawn-v1.5",a.svgText,["authored-layout-contract","chip-baseline-offset-five","single-nonoverlapping-truth-strip"])}
...
function chartMarks(model){
  ...
  else if(model.kind==="sync") {const ys={"Mac A":390,"Windows B":462,"服务":534,"目标态示例":606};r.forEach((item,i)=>{const y=ys[item.track]??390+i*68,symbol=item.status==="冲突示例"?"!":item.status==="不可用"?"×":"—",retry=item.action.includes("重试")?" ↻":"";g+=visualRecord(item,tx(64,y-8,item.track,14,C.text,750)+line(210,y,1260,y,C.border,3)+rect(470+i*150,y-18,250,36,C.softWarn,C.warning,18,`data-encode-field="status" data-encode-value="${esc(item.status)}"`)+tx(595+i*150,y-8,`${symbol} ${item.status} · ${item.action}${retry}`,12,C.warning,700,"middle"))})}
...
for(const model of models.charts)add(`${model.id}.svg`,"chart",model.surface,"shared-structured-model-v1.5",chartBoard(model),["visual-table-model-bidirectional-equality","explicit-model-field-visual-channel-legend-table-contract","table-body-at-least-14","complete-simplified-chinese"]);
...
add("24-components-states.svg","standard","mixed","redrawn-v1.5",components(),["pill-baseline-offset-five","actual-buttons-at-least-44"]);
...
function zoom200(){
  ...
  b+=box("zoom-200-button",784,798,480,88,C.action,C.action,16,`stroke-width="2" data-token="control"`)+`<text x="1024" y="847" font-size="32" fill="#FFFFFF" font-weight="750" text-anchor="middle" dominant-baseline="middle" data-layout-text="true" data-container-ref="zoom-200-button" data-content-key="button" data-authored-baseline-offset="5">${esc(zoomContent[5][1])}</text>`;
...
add("28-accessibility-zoom-200.svg","responsive","private","redrawn-v1.5",zoom200(),["same-content-reflow","eight-token-groups-exactly-two-times","conservative-authored-safe-insets"]);
...
const localizeVisibleText=(source)=>source.replace(/(<(?:text|title|desc)\b[^>]*>)([^<]*)(<\/(?:text|title|desc)>)/g,(_,open,value,close)=>{
  let next=value
    ...
    .replace(/CR-UI-002 v1\.\d/g,"CR-UI-002 v1.5");
...
for(const asset of previousResolved.assets){
  ...
  if(!/(?:connector|conditional)/i.test(visible))continue;
  add(asset.name,asset.category,asset.surface,"localized-visible-copy-v1.5",localizeVisibleText(source),["visible-connector-and-conditional-localized","historical-layout-preserved"]);
...
if(replacements.length!==35)throw new Error(`expected 35 replacements, got ${replacements.length}`);
...
const overlay={schema_version:1,version:"1.5",generated_at:"2026-08-16T01:20:00+08:00",base_version:"1.4",replacement_count:35,truth_boundary:models.truth_boundary,authored_layout_contract:"authored-layout-contract.json",authored_layout_contract_sha256:sha256(path.join(here,"authored-layout-contract.json")),assets:overlayAssets};
...
const resolvedAssets=previousResolved.assets.map(asset=>{
  if(byName.has(asset.name)){const next=byName.get(asset.name);const baseSvg=path.resolve(previousRoot,asset.svg_path),basePng=path.resolve(previousRoot,asset.png_path);return {...next,resolution:"replaced-by-v1.5-overlay",base_svg_sha256:sha256(baseSvg),base_png_sha256:sha256(basePng)};}
  const svgPath=path.resolve(previousRoot,asset.svg_path),pngPath=path.resolve(previousRoot,asset.png_path);return {...asset,resolution:"reused-immutable-v1.4-resolved-by-sha",svg_path:path.relative(here,svgPath),png_path:path.relative(here,pngPath),svg_sha256:sha256(svgPath),png_sha256:sha256(pngPath)};
});
const resolved={schema_version:1,version:"1.5",generated_at:"2026-08-16T01:20:00+08:00",base_resolved_manifest:"../release-completeness-v1.4/resolved-manifest.json",base_resolved_manifest_sha256:sha256(path.join(previousRoot,"resolved-manifest.json")),overlay_manifest:"overlay-manifest.json",overlay_manifest_sha256:sha256(path.join(here,"overlay-manifest.json")),chart_model:"resolved-chart-models.json",chart_model_sha256:sha256(path.join(here,"resolved-chart-models.json")),count:resolvedAssets.length,replaced:35,reused:14,truth_boundary:models.truth_boundary,review_contract:{machine_validation:"machine-verifiable-authored-structure-and-raster-integrity",font_glyph_geometry:"not-machine-claimed",independent_visual_review:"pending-root-coordinator-round-6",downstream_route_authorized:false},assets:resolvedAssets};
fs.writeFileSync(path.join(here,"resolved-manifest.json"),JSON.stringify(resolved,null,2)+"\n","utf8");
console.log(JSON.stringify({status:"generated",version:"1.5",overlay_assets:35,resolved_assets:49,replaced:35,reused:14},null,2));
