...
> 变更编号：`prd-20260804-frontend-career-radar-001`
> 入场审批：`approval-20260804-prd-entry`
...
> 交付状态：待超级无敌帅超超总审核
> 停止门：`prd-review`
> 本版修订：新增用户信息源文本输入、来源／内容分类、与既有调研整合及敏感信息处理要求
...
1. MVP 仍先交付**可在浏览器完整阅读的 Web 内容**；用户信息源输入作为紧随其后的独立交互切片，不得反向阻塞只读内容交付。正式后端服务、在线采集、账号体系和生产部署继续后置。
...
- 后端 API、数据库、数据管道、模型服务、任务调度、生产监控和正式部署。
...
| 为追求 Web 预览跳过审核门 | 返工与治理失效 | 本 PRD 完成后停在 `prd-review`；不得自动启动 UI/UX 或开发 |
...
当前停止在 `prd-review`。超级无敌帅超超总可选择：**通过并自动授权固定 04 UI/UX设计师制作 UI 提示词 / 修改 / 打回**。一次“通过”只授权 UI 提示词一个交付单元；提示词完成后必须重新停门，不代表 UI 设计、架构、开发或发布已获批准。

## 16. 产品经理自查

- [x] 产品定位、目标用户假设和核心价值清晰。
- [x] “职业方向总览 → 技术栈全景”固定为前两层。
- [x] MVP 明确先交付可浏览 Web 内容，后端服务和正式部署后置。
