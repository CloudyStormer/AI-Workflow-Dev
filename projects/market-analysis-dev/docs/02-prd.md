...
Frontend Career Radar 是一个面向前端开发者的**证据型职业成长浏览产品**：先帮助用户理解可选择的职业方向，再展开对应的技术栈全景，并把每个关键判断与来源、日期、地区、岗位层级、样本边界和置信度关联起来。

### 1.2 本轮产品决定

1. MVP 先交付**可在浏览器完整阅读的 Web 内容**，不依赖后端服务、在线采集、账号体系或正式生产部署。
...
5. 无用户个人证据时，所有个人能力、差距和建议状态均为“未知／待补充”，不得生成竞争力分数或把模型推断写成事实。
6. 后端服务、持续数据采集、个人证据保存、账号、数据库、模型服务和正式部署全部后置，须分别经过后续产品、UI/UX、架构及高风险授权门。
...
`docs/00-intake.md` 当前不存在；没有用户访谈、个人能力证据、目标地区／层级、时间预算、隐私授权或付费意愿数据。因此：
...
- 账号、登录、简历上传、GitHub／学习平台接入、真实个人档案或隐私数据保存。
...
| F10 | 内容反馈 | 对方向、技术栈和证据解释提交结构化反馈 | P1 | 否；首版可由人工访谈代替 |
| F11 | 个人证据录入 | 用户自述与可核验证据的手动录入、编辑和删除 | P1 | 否；需隐私与保存方案 |
| F12 | 个性化差距 | 基于已授权个人证据生成可解释的差距和路线 | P2 | 否；需校准和独立审批 |
| F13 | 连续市场更新 | 合法来源清单、采集、去重、过期和趋势序列 | P2 | 否；需逐源权利审查和后端服务 |
| F14 | 账号与同步 | 账号、跨设备保存、导入和审计 | P2 | 否；涉及隐私、权限和后端 |
| F15 | 正式发布 | 生产部署、域名、监控、回滚和公开访问 | P2 | 否；需具体高风险授权 |
...
- **功能描述**：说明形成个人能力判断前需要哪些证据；MVP 不采集、不保存真实个人资料。
...
- **边界情况**：涉及雇主机密、个人隐私或不可公开材料时，只记录可否使用和公开限制；MVP 不要求上传内容。
...
- **AC-16**：Given 用户未来只提供自述，When 系统显示该信息，Then 明确标为“用户自述、待核验”，不升级为已掌握事实。
...
- **AC-28**：Given MVP 运行，When 检查网络与数据行为，Then 不需要登录、真实个人数据上传、自动招聘采集或受限来源访问。
- **AC-29**：Given 首个 Web 切片完成，When 团队报告结果，Then 只能称为“可浏览预览／内容切片”，除非另有正式生产发布审批和验证，不得称为已上线。
- **AC-30**：Given 任一后置能力被提出，When 它涉及后端、个人数据、生产发布、付费或外部发送，Then 必须进入新的产品范围与对应审批门，不得并入本 MVP 实施。
...
4. 收集用户愿意提供的个人证据类型与隐私边界，不默认要求连接外部账号。
5. 在形成有效访谈或行为样本前，不设置留存率、付费率或个性化效果目标。
...
以下输入不是批准本 PRD 的前置条件，但决定后续个人化范围：
