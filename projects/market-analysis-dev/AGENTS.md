...
19. 阶段状态由对应固定角色本人报告：当前角色在自己的固定任务独立交付；获批路由后，下一角色在自己的固定任务宣布入场、范围和停止门；`00 包工头`只监督汇总，不代替角色报到或交付。
20. 项目必须保留 `ui/`，统一存放 UI/UX 提示词、设计说明、原型、生成界面和视觉审核产物；新产物登记路径与哈希，审核中的旧产物不得中途移动。
21. 超级无敌帅超超总在固定角色本人任务中明确本项目、范围和动作时，视为该角色该工作单元的一次性入场或继续授权，无需 `00 包工头`转发，并具有内部最高业务调度优先级；但不得自动批准待审产物、解冻其他工作、授权下游或跳过原流程。执行前记录安全恢复点和返回点，完成后回原队列或审核门；高风险动作仍须本次具体授权。
