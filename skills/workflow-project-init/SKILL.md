...
2. 先向齐总提交初始化或收编方案；只有齐总明确批准后才能写入。
3. 不移动、删除或重命名已有入口，除非齐总单独批准迁移。
...
9. 整理完成后更新 `workflow/skill-lock.yaml`，运行结构验证和 Skill 验证，再请求齐总审核。
...
以运行方式为准，不以目录外观为准。若改变 Profile 会影响构建或部署，停止并请求齐总决定。
