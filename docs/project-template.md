...
模板还必须支持“用户直达固定角色”：超级无敌帅超超总在角色本人任务中明确项目、范围和动作后，该消息即为该角色该临时工作单元的一次性授权，无需 `00 包工头`转发。角色记录原状态、安全恢复点和返回点后可安全插队，完成后恢复原队列；直达不隐含待审产物通过、解冻、下游授权或跳过产品/UI 回退链，高风险动作仍须本次具体授权。
