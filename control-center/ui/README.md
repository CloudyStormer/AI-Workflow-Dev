...
- `01-control-center-ui-design-prompt.md`：真实监管后端状态 UI Prompt 增量已完成，当前停在 `ui-prompt-review` 等待超级无敌帅超超总审核。
- 当前 Prompt 版本：v1.1。
...
当前增量的实现冲突按“已批准 PRD v1.0 → 待审 Prompt v1.1 → 全局简体中文版规则 → 历史图片视觉参考”的顺序裁决。图片中的示例数据不得冒充实时事实，必须继续标注为演示数据或待接入；真实监管服务尚未实现。

固定 `04 UI/UX设计师` 本轮只交付 Prompt v1.1 并停在 `ui-prompt-review`，不生成设计稿、原型或图片，不修改现有代码审查结论，也不启动架构或开发。新产物必须登记路径、版本、状态和 SHA256；目录或文件存在不代表设计或实现已经完成。
